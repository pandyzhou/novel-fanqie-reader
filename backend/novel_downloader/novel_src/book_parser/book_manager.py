...
{xhtml_content}
    ...
    def remove_generated_output(self):
        """Remove generated export files for this novel."""
        safe_book_name = re.sub(r'[\\/*?:"<>|]', "_", self.book_name)
        for extension in ("epub", "txt", "pdf"):
            output_file = self.save_dir / f"{safe_book_name}.{extension}"
            if output_file.exists():
                try:
                    os.remove(output_file)
                    self.logger.info(f"Removed generated output: {output_file}")
                except OSError as e:
                    self.logger.warning(f"Failed to remove generated output {output_file}: {e}")

    def finalize_download(self, chapters: List[Dict], failed_count: int):
        """Saves final status and generates export only when all chapters succeeded."""
        self.save_download_status()

        if failed_count > 0:
            self.remove_generated_output()
            self.logger.warning(
                f"Skipping export generation for {self.book_name} because {failed_count} chapters failed or are missing."
            )
            return False
        ...
        if not self.config.bulk_files and self.config.novel_format == "epub":
            ...
            try:
                ...
                for chapter_meta in chapters:
                    ...
                    if not data:
                        raise ValueError(f"Missing downloaded content for chapter {chapter_id}")

                    ch_title, ch_content = data
                    is_error_content = isinstance(ch_content, str) and (
                        ch_content == "Error" or "Error" in ch_content
                    )
                    if is_error_content:
                        raise ValueError(f"Chapter {chapter_id} still contains error marker: {ch_content}")

                    xhtml_chapter_content = f"<h1>{ch_title.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')}</h1><div>{ch_content}</div>"
                    epub.add_chapter(ch_title, xhtml_chapter_content)

                epub.generate(str(output_file))
        ...
        if self.config.auto_clear_dump and self.end:
