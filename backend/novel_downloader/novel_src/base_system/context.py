...
class Config(BaseConfig):
    ...
    use_proxy_api: bool = Field(default=False, description="使用代理API（可选）")
    use_thirdparty_api: bool = Field(default=True, description="使用第三方番茄API（cenguigui）")
    # 自定义内容API（通过配置启用）
    use_custom_content_api: bool = Field(default=False, description="使用自定义内容API")
    custom_content_api_base: str = Field(default="", description="自定义内容API基址（例如 http://host/fqy/api_v2.php）")
    custom_content_api_key: str = Field(default="", description="自定义内容API的X-API-Key")
    iid: str = Field(default="", description="自动生成")
    iid_spawn_time: str = Field(default="", description="iid生成时间戳")
    api_endpoints: list = Field(
        default=[],
        description="第三方/自建API列表（如 cenguigui 基址）",
    )
